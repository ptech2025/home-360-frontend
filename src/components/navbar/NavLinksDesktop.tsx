      ...
      {navLinksArr.map((link) => (
        <li key={link.name} className="flex items-center justify-center">
          <Link
            ...
            className={cn(
              "inline-block py-4 data-[state=active]:text-main-blue hover:data-[state=inactive]:text-main-blue/80 data-[state=inactive]:text-[#838E9E] font-circular-medium  cursor-pointer justify-center text-base font-semibold data-[state=active]:border-dark-orange border-b-2 data-[state=inactive]:border-transparent transition-colors "
            )}
            href={link.href}
          >
            {link.name}
